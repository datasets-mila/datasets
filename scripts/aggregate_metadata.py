...
def use_git_dir(path):
    try:
        _git_dir = os.environ["GIT_DIR"]
    except KeyError:
        pass
...
def ds_flag_as_uptodate(root_path, ds_path):
    abs_path = os.path.join(root_path, ds_path)
    try:
    ...
    if ds_path != '.' and subds_parent is None:
        subds_parent = copy.deepcopy(subds_json)
        subds_parent["path"] = os.path.relpath(".", ds_path)
        subds_parent["name"] = ".."
...
for path in ds.subdatasets(result_xfm='relpaths'):
    ...
    path += ".var"
    var = sorted(glob.glob(os.path.join(path, "*")))
    if var:
        subds_var[path] = var
...
if subds_to_update:
    for subds in list(subds_to_update):
        try:
            ls(subds, recursive=True, all_=True, long_=True, json="file")
            ds_flag_as_uptodate(ds.path, subds)
        except NotADirectoryError as e:
            print(e, file=sys.stderr)
            subds_to_update.remove(subds)

if subds_to_update or ds_need_update(ds.path, '.'):
    with use_git_dir(os.path.join(ds.path, ".git")):
        ds.aggregate_metadata()
    subprocess.run([os.path.join(ds.path, "scripts/aggregate_metadata_root.sh")], check=True)
    ds_flag_as_uptodate(ds.path, '.')

exclude_set = {".dataset_template", "ava-kinetics", "openwebtext", "webtext"}

ds_json = load_json(ds.path, '/')
ds_json["nodes"] = [node for node in ds_json["nodes"]
        if len(node["name"]) == 1 or
        (node["name"][0] != "." and node["name"] not in exclude_set)]
